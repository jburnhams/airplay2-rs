...
impl ReceiverOutput {
    ...
    pub fn analyze_audio_detailed(&self) -> Result<AudioAnalysis, Box<dyn std::error::Error>> {
        ...
        Ok(AudioAnalysis {
            num_samples: result.num_frames * raw_audio.channels as usize,
            duration: result.duration.as_secs_f32(),
            sample_rate: raw_audio.sample_rate as f32,
            rms: result.rms,
            peak: result.peak,
            crest_factor: result.crest_factor,
            estimated_frequency: result.measured_frequency,
            zero_crossings: 0, /* Legacy field, not computed identically anymore but typically
                                * unused */
        })
    }
}
