...
async fn test_raop_handshake_compliance() {
    ...
    match result {
        ...
        Ok(Err(e)) => std::panic::resume_unwind(e.into_panic()), // Propagate panics from background task
        Err(_) => println!("Timeout waiting for client"), // Timeouts are acceptable here depending on OS behavior
    }
}
