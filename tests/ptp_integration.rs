...
async fn test_pending_t3_reset_prevents_stuck_exchange() {
    ...
    let mut msg = PtpMessage::decode(&buf[..len]).unwrap();
    if msg.header.message_type != PtpMessageType::DelayReq {
        // Keep receiving until we get DelayReq
        while let Ok(Ok((len, _))) = tokio::time::timeout(Duration::from_millis(100), homepod_event_sock.recv_from(&mut buf)).await {
            msg = PtpMessage::decode(&buf[..len]).unwrap();
            if msg.header.message_type == PtpMessageType::DelayReq {
                break;
            }
        }
