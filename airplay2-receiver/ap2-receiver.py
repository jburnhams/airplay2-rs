...
class AP2Handler(http.server.BaseHTTPRequestHandler):
    ...
    def do_SETUP(self):
        ...
        if self.headers["Transport"]:
            ...
            stream = {
                'audioFormat': self.sdp.AirplayAudFmt if hasattr(self.sdp, 'AirplayAudFmt') else (1 << 11),  # Default to PCM_44100_16_2 (1<<11)
                ...
                'shk': self.aeskeyobj.aeskey if self.aeskeyobj else (
                    self.ecdh_shared_key[:32] if self.ecdh_shared_key else b"\x00" * 32
                ),
                'shiv': self.aeskeyobj.aesiv if self.aeskeyobj else b"\x00" * 16,
                'spf': int(self.sdp.spf) if self.sdp.spf else 352,
