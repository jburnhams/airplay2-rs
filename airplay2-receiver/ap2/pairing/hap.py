...
class CRUD_Store:
    ...
    def create_entry(self, _id: bytes, _which: str, _val):
        if not self.has_entry(_id):
            self.store[_id.decode()] = {f"{_which}": _val}
        else:
    ...
    def update_entry(self, _id: bytes, _which: str, _val):
        ...
        self.store[_id.decode()].update({f"{_which}": _val})
        self.json_handler.put_store(self.store)
    ...
    def delete_entry(self, _id: bytes, _which=None):
        if not _which:
            ...
            ret = self.store[_id.decode()].pop(f"{_which}")
            self.json_handler.put_store(self.store)
            return ret
    ...
    def set_bytes(self, _id: bytes, _which: str, _val: bytes):
        if _val is not None:
            self.create_entry(_id, _which, base64.standard_b64encode(_val).decode())
        else:
            self.delete_entry(
                _id,
                _which,
    ...
    def get_bytes(self, _id: bytes, _which: str):
        return base64.standard_b64decode(self.read_entry(_id, _which))
    ...
    def set_string(self, _id: bytes, _which: str, _val: str):
        if _val is None or _val == "":
            ...
            self.create_entry(_id, _which, _val)
...
class Hap:
    def __init__(self, identifier, isDebug=False):
        ...
        if self.ltsk.has_entry(self.accessory_pairing_id):
            self.logger.debug(
                f"Loading ed25519 keypair for own ID: {self.accessory_pairing_id.decode('utf-8')}"
            )
            ...
            self.logger.debug(
                f"Generating new ed25519 keypair for own ID: {self.accessory_pairing_id}"
            )
            ...
            self.accessory_ltpk = self.accessory_ltsk.public_key().public_bytes(
            ...
            self.ltsk.set_ltpk(self.accessory_pairing_id, self.accessory_ltpk)
    ...
    def pair_setup(self, req, pw):
        ...
        if (
            ...
            and req[Tlv8.Tag.METHOD] == PairingMethod.PAIR_SETUP
            and Tlv8.Tag.FLAGS in req
            and PairingFlags(flags) == PairingFlags.TRANSIENT
        ):
            self.transient = True
            self.pair_setup_steps_n = 2
        elif (
            req[Tlv8.Tag.STATE] == PairingState.M1
            and req[Tlv8.Tag.METHOD] == PairingMethod.PAIR_SETUP_AUTH
            and Tlv8.Tag.FLAGS in req
            and PairingFlags(flags) == PairingFlags.TRANSIENT
        ):
    ...
    def pair_add_m1_m2(self, req):
        ...
        if permissions != HomeKitPermissions.Admin:
            ...
            """
            self.logger.debug(
                "Controller does not have admin bit set in local pairings list"
            )
            return [
                Tlv8.Tag.STATE,
                PairingState.M2,
                Tlv8.Tag.ERROR,
                PairingErrors.AUTHENTICATION,
            ]
        ...
        if self.pairings.has_entry(_id):
            ...
            try:
                ...
                self.logger.debug(
                    "pair-add was unable to save pairing data to persistent store:"
                )
    ...
    def pair_setup_m5_m6(self, encrypted):
        ...
        self.accessory_shared_key = self.ctx.session_key
        self.encrypted = True
        return [Tlv8.Tag.STATE, PairingState.M6, Tlv8.Tag.ENCRYPTEDDATA, enc_tlv + tag]
    ...
    def pair_setup_m5_m6_2(self, dec_tlv):
        ...
        prk = hkdf.hkdf_extract(
            b"Pair-Setup-Controller-Sign-Salt", self.ctx.session_key
        )
    ...
    def pair_verify_m1_m2(self, client_public):
        ...
        self.accessory_shared_key = self.accessory_random.exchange(
            x25519.X25519PublicKey.from_public_bytes(client_public)
        )

        accessory_info = (
            self.accessory_random_public + self.accessory_pairing_id + client_public
        )
        accessory_sig = self.accessory_ltsk.sign(accessory_info)
    ...
    def pair_verify_m3_m4(self, encrypted):
        ...
        device_info = (
            self.client_curve_public + device_id + self.accessory_random_public
        )
...
class HAPSocket:
    ...
    def __init__(self, sock, shared_key):
        ...
        # Derive control keys manually to ensure HKDF-SHA512 correctness
        # HKDF-Extract
        salt = b"Control-Salt"
        # Standard: hmac(key=salt, msg=ikm)
        prk = hmac.new(salt, shared_key, hashlib.sha512).digest()
        ...
        self.read_key = expand(prk, b"Control-Write-Encryption-Key", 32)
        self.write_key = expand(prk, b"Control-Read-Encryption-Key", 32)
        
        self.lock = threading.RLock()

    def _get_nonce(self, is_input):
        # RFC 7539: 12 bytes nonce (96 bits)
        # HAP: 8 bytes counter (64 bits) padded with 4 bytes of 0s
        # The counter is little endian
        # 4 bytes of 0s
        # 8 bytes of counter (little endian)
        # Total 12 bytes
        # PyCryptodome ChaCha20_Poly1305 expects 12 bytes nonce

        # NOTE: This implementation relies on the fact that we create a new cipher for each packet
        # because the nonce is fixed at creation time in PyCryptodome's new() method.
        # But wait, we can't recreate cipher every time if it holds state?
        # ChaCha20Poly1305 is stateless regarding key/nonce combination, but we need unique nonce.

        # Actually, PyCryptodome's ChaCha20_Poly1305.new() creates a cipher object.
        # To encrypt/decrypt, we call encrypt_and_digest or decrypt_and_verify.
        # The state is not maintained between calls for the counter?
        # We need to manage the nonce ourselves.
        pass

    def _get_nonce_bytes(self, count):
        # 4 bytes 0 + 8 bytes counter (little endian)
        return b"\x00\x00\x00\x00" + struct.pack("<Q", count)

    def recv(self, buflen):
        # We need to read the encrypted length first (2 bytes)
        # But wait, HAP frames are:
        # <Length: 2 bytes> <Encrypted Data> <Auth Tag: 16 bytes>
        # The Length includes the data but not the tag? No.
        # Length is length of ciphertext (data).

        # We can't easily implement recv(buflen) to behave exactly like socket.recv
        # because we receive in frames.
        # We'll read one frame and return its content.

        # Reading 2 bytes length (encrypted? No, length is plaintext AA in some versions? No, HAP is specific)
        # HAP:
        # <Little Endian Length (2 bytes)> <Ciphertext (Length bytes)> <AuthTag (16 bytes)>


        # Length is unencrypted.
        ...
        while len(payload) < to_read:
            chunk = self.socket.recv(to_read - len(payload))
            if not chunk:
                break
            payload += chunk
        ...
        # Decrypt
        nonce = self._get_nonce_bytes(self.in_count)
        cipher = ChaCha20_Poly1305.new(key=self.read_key, nonce=nonce)
        cipher.update(length.to_bytes(2, byteorder="little"))
    ...
    def recv_into(self, buffer, nbytes=None):
        data = self.recv(nbytes if nbytes else len(buffer))
        if not data:
            return 0
    ...
    def sendall(self, data):
        # We must chunk data into frames if it's too large (max 1024 bytes per HAP spec usually)
        # But here we assume chunks are small enough or we chunk them.
        ...
        with self.lock:
            ...
            while total_sent < len(data):
                ...
                nonce = self._get_nonce_bytes(self.out_count)
                cipher = ChaCha20_Poly1305.new(key=self.write_key, nonce=nonce)
                cipher.update(length.to_bytes(2, byteorder="little"))
                ciphertext, tag = cipher.encrypt_and_digest(chunk)

                # <Length 2 bytes> <Ciphertext> <Tag 16 bytes>
    ...
    def makefile(self, mode="r", bufsize=-1):
        import io
        print(f"[HAPSocket] makefile mode={mode}")
        if 'b' not in mode:
            mode += 'b'
        if 'r' in mode:
            res = io.BufferedReader(self, buffer_size=bufsize if bufsize > 0 else io.DEFAULT_BUFFER_SIZE)
            print(f"[HAPSocket] makefile returning Reader: {res}")
            return res
        if 'w' in mode:
            # Return unbuffered writer (self) to ensure immediate send
            print(f"[HAPSocket] makefile returning Writer: self")
            return self
        return None
