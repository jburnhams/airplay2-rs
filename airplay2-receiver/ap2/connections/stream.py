...
class Stream:
    ...
    def __init__(
        ...
        # A uint64:
        self.streamConnectionID = (
            stream["streamConnectionID"] if "streamConnectionID" in stream else None
        )
        # A boolean:
        self.supportsDynamicStreamID = (
            stream["supportsDynamicStreamID"]
            if "supportsDynamicStreamID" in stream
            else None
        )
        ...
        if self.streamtype == Stream.REALTIME or self.streamtype == Stream.BUFFERED:
            ...
            raw_key = stream["shk"] if "shk" in stream else b"\x00" * 32
            # Ensure key is exactly 32 bytes for ChaCha20-Poly1305
            if len(raw_key) > 32:
                print(f"[Stream] Truncating session key from {len(raw_key)} to 32 bytes")
                self.session_key = raw_key[:32]
            elif len(raw_key) < 32:
                print(f"[Stream] Padding session key from {len(raw_key)} to 32 bytes")
                self.session_key = raw_key + b"\x00" * (32 - len(raw_key))
            else:
        ...
        if self.streamtype == Stream.REALTIME:
            ...
            self.server_control = (
                stream["controlPort"] if "controlPort" in stream else None
            )
