...
use crate::protocol::crypto::{X25519KeyPair, X25519PublicKey, derive_key, ChaCha20Poly1305Cipher, Nonce};
...
fn test_full_pair_setup_flow() {
    ...
    let verifier = srp_client.process_challenge(b"Pair-Setup", b"1234", salt, server_public).unwrap();
...
fn test_pair_setup_wrong_password() {
    ...
    let srp_client = SrpClient::new(&SrpParams::RFC5054_3072).unwrap();
    let client_public = srp_client.public_key();
    let verifier = srp_client.process_challenge(b"Pair-Setup", b"wrong_pass", salt, server_public).unwrap();
    let client_proof = verifier.client_proof();
    ...
    assert_eq!(err_tlv.get(TlvType::Error).and_then(|v| v.first()).copied(), Some(2));
}
...
fn test_pair_verify_flow() {
    ...
    // --- Run Setup ---
    let m1 = TlvEncoder::new().add_state(1).add_byte(TlvType::Method, 0).build();
    ...
    let verifier = srp_client.process_challenge(b"Pair-Setup", b"1234", salt, server_pub).unwrap();
    let client_proof = verifier.client_proof();
    ...
    assert_eq!(verify_res_m1.new_state, PairingServerState::VerifyWaitingForM3);
...
fn test_pair_verify_bad_signature() {
    ...
    let m3 = TlvEncoder::new().add_state(3).add(TlvType::PublicKey, client_srp_pub).add(TlvType::Proof, client_proof).build();
    let _ = server.process_pair_setup(&m3);
    ...
    let server_curve_pub = X25519PublicKey::from_bytes(&arr).unwrap();
    let shared_secret = client_curve_kp.diffie_hellman(&server_curve_pub);
    let session_key = derive_key(Some(b"Pair-Verify-Encrypt-Salt"), shared_secret.as_bytes(), b"Pair-Verify-Encrypt-Info", 32).unwrap();
    ...
    let vm3 = TlvEncoder::new().add_state(3).add(TlvType::EncryptedData, &encrypted_vm3).build();
    let res_vm3 = server.process_pair_verify(&vm3);
