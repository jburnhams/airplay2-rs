use crate::protocol::sdp::parser::SdpParser;
...
#[test]
fn test_detect_codec_aac_eld() {
    let sdp_str = r"v=0
m=audio 0 RTP/AVP 96
a=rtpmap:96 mpeg4-generic ELD
";
    let sdp = SdpParser::parse(sdp_str).unwrap();
    let audio = sdp.audio_media().unwrap();
    let codec = detect_codec(audio).unwrap();
    assert_eq!(codec, AudioCodec::AacEld);
}

#[test]
fn test_detect_codec_aac_lc() {
    let sdp_str = r"v=0
m=audio 0 RTP/AVP 96
...
#[test]
fn test_detect_codec_mp4a_latm() {
    let sdp_str = r"v=0
m=audio 0 RTP/AVP 96
...
fn test_detect_codec_unknown() {
    let sdp_str = r"v=0
m=audio 0 RTP/AVP 96
a=rtpmap:96 UnknownCodec
";
    let sdp = SdpParser::parse(sdp_str).unwrap();
    let audio = sdp.audio_media().unwrap();
    let codec = detect_codec(audio);
    assert_eq!(codec, None);
}

#[test]
fn test_parse_encryption_invalid_aesiv_base64() {
    let sdp_str = r"v=0
m=audio 0 RTP/AVP 96
a=rsaaeskey:VGhpcyBpcyBhIHRlc3Qga2V5IHRoYXQgaXMgdXNlZCBmb3IgdGVzdGluZw==
a=aesiv:!!invalid!!base64!!
";
    let sdp = SdpParser::parse(sdp_str).unwrap();
    let audio = sdp.audio_media().unwrap();
    let res = parse_encryption(audio);
    assert!(res.is_err());
}

#[test]
fn test_parse_encryption_invalid_rsaaeskey_base64() {
    let sdp_str = r"v=0
m=audio 0 RTP/AVP 96
a=rsaaeskey:!!invalid!!base64!!
a=aesiv:MDEyMzQ1Njc4OWFiY2RlZg==
";
    let sdp = SdpParser::parse(sdp_str).unwrap();
    let audio = sdp.audio_media().unwrap();
...
#[test]
fn test_parse_encryption_invalid_aesiv_length() {
    let sdp_str = r"v=0
m=audio 0 RTP/AVP 96
a=rsaaeskey:VGhpcyBpcyBhIHRlc3Qga2V5IHRoYXQgaXMgdXNlZCBmb3IgdGVzdGluZw==
a=aesiv:MDEyMzQ1Njc4OQ==
...
#[test]
fn test_extract_stream_parameters_missing_audio_media() {
    let sdp_str = r"v=0
m=video 0 RTP/AVP 96
a=rtpmap:96 H264
";
    let sdp = SdpParser::parse(sdp_str).unwrap();
    let res = extract_stream_parameters(&sdp, None);
    assert!(res.is_err());
}

#[test]
fn test_extract_stream_parameters_missing_rtpmap() {
    let sdp_str = r"v=0
m=audio 0 RTP/AVP 96
a=fmtp:96 352 0 16 40 10 14 2 255 0 0 44100
";
...
#[test]
fn test_extract_stream_parameters_alac_missing_fmtp() {
    let sdp_str = r"v=0
m=audio 0 RTP/AVP 96
a=rtpmap:96 AppleLossless
";
...
#[test]
fn test_extract_stream_parameters_aac() {
    let sdp_str = r"v=0
m=audio 0 RTP/AVP 96
a=rtpmap:96 mpeg4-generic
";
    let sdp = SdpParser::parse(sdp_str).unwrap();
    let params = extract_stream_parameters(&sdp, None).unwrap();
    assert_eq!(params.codec, AudioCodec::AacLc);
    assert_eq!(params.sample_rate, 44100);
    assert_eq!(params.bits_per_sample, 16);
    assert_eq!(params.channels, 2);
    assert_eq!(params.frames_per_packet, 352);
...
#[test]
fn test_parse_encryption_missing_aesiv() {
    let sdp_str = r"v=0
m=audio 0 RTP/AVP 96
a=rsaaeskey:VGhpcyBpcyBhIHRlc3Qga2V5IHRoYXQgaXMgdXNlZCBmb3IgdGVzdGluZw==
";
