...
impl TlvEncoder {
    ...
    ///
    /// # Panics
    /// Panics if a chunk size exceeds 255 bytes, which should not happen due to `chunks(255)`.
    #[must_use]
    pub fn add(mut self, tlv_type: TlvType, value: &[u8]) -> Self {
        ...
        for chunk in value.chunks(255) {
            ...
            // Safe because chunk size is limited to 255
            self.buffer
                .push(u8::try_from(chunk.len()).expect("chunk size <= 255"));
...
impl TlvDecoder {
    ...
    ///
    /// # Errors
    /// Returns `TlvError` if the data is malformed.
    ...
    ///
    /// # Errors
    /// Returns `TlvError::MissingField` if the field is not present.
    ...
    ///
    /// # Errors
    /// Returns `TlvError` if the field is missing or invalid.
