...
impl TransientPairing {
    ...
    pub fn new() -> Result<Self, PairingError> {
        ...
        let signing_keypair = Ed25519KeyPair::generate(); // Note: updated to not return Result as per memory
    ...
    ///
    /// # Errors
    /// Returns `PairingError` if the state is invalid or data processing fails.
    ...
    ///
    /// # Errors
    /// Returns `PairingError` if the state transition is invalid.
        ...
        match self.state {
            ...
            PairingState::Complete => {
                if let Some(keys) = &self.session_keys {
                    Ok(PairingStepResult::Complete(keys.clone()))
                } else {
                    Err(PairingError::InvalidState {
                        expected: "session keys".to_string(),
                        actual: "none".to_string(),
                    })
                }
            }
