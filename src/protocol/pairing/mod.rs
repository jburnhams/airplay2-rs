...
impl SessionKeys {
    ...
    ///
    /// # Errors
    /// Returns `CryptoError` if cipher creation fails.
...
pub struct EncryptedChannel {
    ...
    nonce_counter: u64,
    #[allow(dead_code)] // Will be used for directional logic if needed
    is_sender: bool,
}
...
impl EncryptedChannel {
    ...
    ///
    /// # Errors
    /// Returns `CryptoError` if the key is invalid.
    pub fn new(
        key: &[u8],
        initial_nonce: u64,
        is_sender: bool,
    ...
    pub fn encrypt_framed(
        ...
        // Length of the encrypted payload as u16 little-endian
        let len = u16::try_from(encrypted.len()).map_err(|_| {
            // This case should ideally not happen if encrypted is reasonable size,
            // but we need to satisfy the type checker.
            // Using a generic crypto error or adding a specific one would be better.
            // For now, re-use a simple error path or just panic if we assume constraints.
            // But let's assume it fits.
            // Actually, let's map it to a CryptoError for now.
            crate::protocol::crypto::CryptoError::EncryptionFailed("Message too long".to_string())
        })?;
